...
<html>
	...
	<body>
			<?php
				...
				//Prepare the query
				$sql = "SELECT Loaned.LoanID, Toys.ToyID, Toys.Toy, Toys.Image, Members.FName, Members.LName, Members.Email, Members.Telephone, Members.Mobile, Loaned.Due , Loaned.ReturnDay, Toys.Status FROM Toys,Loaned,Members WHERE Toys.ToyID = Loaned.ToyID AND Members.CustomerID = Loaned.CustomerID ORDER BY LName;";
				...
				if (!$result) {
					...
					if ($num_rows == 0) {
						...
						while ($row = mysqli_fetch_array($result,MYSQL_ASSOC)) {
							...
							echo '<td>'.$row['Due'].'</td>';
							echo '<td>'.$row['ReturnDay'].'</td>';
							echo '<td>'.$row['Status'].'</td>';
							
							//Calculate date difference
							$datetime1 = date_create($row['ReturnDay']);
							$datetime2 = date_create($row['Due']);
							...
							echo '<td>'.'<a href="inchange.php?id='.$row['LoanID'].'&'.'id2='.$row['ToyID'].'">Set Loan Status to IN</a>'.'<br>'.'<br>'.'</td>'; //Adds link to update fields
							echo '</tr>';
						}
						echo '</tbody>';
